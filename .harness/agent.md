...
2. **Route by layer, not by file extension.** Match the task to the right rein:
   ...
   - Unity APIs, VContainer DI, scenes, ScriptableObjects, build pipeline, editor tools → `unity-expert`
   - APK boyut optimizasyonu (Addressables, PAD, texture sıkıştırma, code stripping) → `unity-expert`
   - Google Play Platform (Play Games Services, Play Instant, Play Integrity, In-App Review) → `unity-expert`
   - Görsel efektler, PrimeTween animasyonları, UI/UX akışları → `ui-vfx-expert`
   - Erişilebilirlik UI (renk körlüğü modu, yüksek kontrast, desen overlay, reduced motion, screen reader) → `ui-vfx-expert`
   - Haptic feedback, onboarding/tutorial akışı, meta-game altyapısı → `ui-vfx-expert`
   - Any code touching `Assets/Tests/**`, or when the task is "verify the change" → `tester`
   - Güvenlik testleri (save tampering, input spam, leaderboard injection) → `tester`
   ...
   - Lokalizasyon pipeline entegrasyonu → `developer`
   - Reklam SDK entegrasyonu, GDPR/COPPA/ATT consent flow, analitik event plumbing → `liveops-monetization-expert`
   - Güvenlik korumaları (anti-cheat, save tampering) → `liveops-monetization-expert`
...
5. 🛡️ **QA, Security & Edge-Case Specialist** — Unit test edilebilirlik, hacker senaryoları, `NullReferenceException`, input spam. Karamsar: "ya oyuncu saniyede 20 kez tıklarsa?". **🔴 Güvenlik sorumluluğu:** Save dosyası bütünlük kontrolü (checksum/HMAC), bellek manipülasyonu tespiti (memory tampering), leaderboard score injection önleme ve cheat-detection mekanizmaları. Güvenlik açığı olan hiçbir PR'ı onaylamaz.
