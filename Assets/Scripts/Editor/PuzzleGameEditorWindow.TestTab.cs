...
namespace PuzzleGame.Editor
{
    public partial class PuzzleGameEditorWindow
    {
        ...
        private void DrawDebugSection()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (_debugMode)
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        if (GUILayout.Button("Log Bottles", GUILayout.Width(120)))
                        {
                            LogBottleInfo();
                        }
        ...
        private void LogBottleInfo()
        {
            ...
            {
                Debug.Log($"[DEBUG] Bottle #{bottle.BottleIndex}: {bottle.State.LayerCount} layers, {bottle.State.TotalFill:F2} fill");
            }
        }
