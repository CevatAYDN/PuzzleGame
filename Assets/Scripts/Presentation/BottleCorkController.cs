...
namespace PuzzleGame
{
    ...
    /// Extracted from BottleController for SRP (single responsibility = cork).
    /// Animation service is optional — cork drop animation is skipped if null.
    /// </summary>
    public sealed class BottleCorkController
    {
        private readonly Transform _bottleTransform;
        private readonly IAnimationService _animationService;
        private readonly Func<float> _bottleHeightProvider;
        ...
        public BottleCorkController(
            Transform bottleTransform,
            IAnimationService animationService,
            Func<float> bottleHeightProvider,
            Func<float> neckRadiusProvider,
            GameObject existingCork)
        {
            _bottleTransform = bottleTransform ?? throw new ArgumentNullException(nameof(bottleTransform));
            _animationService = animationService; // nullable — cork drop animation is optional
            _bottleHeightProvider = bottleHeightProvider ?? throw new ArgumentNullException(nameof(bottleHeightProvider));
        ...
        public void EnsureCork()
        {
            if (_corkObject == null)
            {
                var child = _bottleTransform.Find("Cork");
                if (child != null)
                {
        ...
        public void AnimateDrop()
        {
            ...
            if (_corkObject != null)
            {
                ...
                _animationService?.AnimateCorkDrop(
                    _corkObject.transform, _bottleHeightProvider(), onComplete: null);
            }
        }
