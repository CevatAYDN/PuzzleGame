...
{
    public class MoldStateReplaceLayersTests
    {
        ...
        public void SetUp()
        {
            _state = new MoldState(maxLayers: 4);
        }
        ...
        public void ReplaceLayers_FiresOnLayersChangedEvent()
        {
            ...
            var newLayers = new[]
            {
                new OreLayer(new DomainColor(1, 0, 0), 0.5f)
            };
        ...
        public void ReplaceLayers_DoesNotFireEvent_WhenInvalid()
        {
            ...
            var invalidLayers = new OreLayer[10];
            for (int i = 0; i < invalidLayers.Length; i++)
                invalidLayers[i] = new OreLayer(new DomainColor(0, 1, 0), 0.1f);
        ...
        public void ReplaceLayers_UpdatesStateCorrectly()
        {
            var newLayers = new[]
            {
                new OreLayer(new DomainColor(1, 0, 0), 0.25f),
                new OreLayer(new DomainColor(0, 1, 0), 0.25f)
            };
            _state.ReplaceLayers(newLayers);
        ...
        {
            int calls = 0;
            System.Action<MoldState> handler = s => calls++;
