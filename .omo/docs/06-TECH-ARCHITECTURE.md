...
| Resources.Load sprawl (28 site) | Major | ✅ **Sprint #16 tamamlandı**: Tüm runtime sıcak yollarındaki (11 site) senkron `Resources.Load` çağrıları `IAssetProvider` asenkron yükleme ve konfigürasyon DI enjeksiyonu (`WobbleConfig` enjeksiyonu, `StreamRenderer` ve `ParticleFactory` asenkron ön yükleme) ile giderildi. |
| Silent Exception Swallowing & try-catch control flow | Medium | ✅ **Sprint #16/17 tamamlandı**: `LocalizationBootstrap` içindeki VContainer `Resolve` kontrol akışı try-catch yerine `TryResolve` ile değiştirildi. `GameSaveManager` içindeki boş catch blokları `MoldLogger.LogWarning` ile uyarı loglarına dönüştürüldü. `AnimationService.Dispose` içindeki gereksiz try-catch kaldırıldı. |
